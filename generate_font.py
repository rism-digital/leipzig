...
def validateGlyph(glyph):
    ...
    if result & 0x1 or True:
        ...
        if report != "":
            print("{} ({:#002x}) issues: {}".format(g.glyphname, glyph.unicode, report))
    return report
...
try:
    ...
    print(f"Can't open font file {fontFileName}")
    sys.exit(1)
...
with open("glyphnames.json") as smuflGlyphnamesFile:
    ...
    codepoint_to_name = {
        data["codepoint"]: name for name, data in smuflGlyphnames.items()
    }
...
for glyph in font:
    ...
    if g.unicode != -1 and g.unicode > 31:
        ...
        validateGlyph(g)
        g.addExtrema()
        bbox = [round(bb / 250.0, 4) for bb in g.boundingBox()]
        ...
        if len(g.anchorPoints) > 0:
            ...
            for item in g.anchorPoints:
                anchors[item[0]] = (
                    round(item[2] / 250.0, 4),
                    round(item[3] / 250.0, 4),
                )
...
jsonFileName = outDir + font.fontname + ".json"
